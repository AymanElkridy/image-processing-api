...
describe('Test endpoint responses', () => {
    ...
    it('gets an error from a non-existing image', async () => {
        const response = await request.get('/image?filename=someimage&width=200&height=200');
        expect(response.status).toBe(400);
    })
    it('gets an existing image', async () => {
        const response = await request.get('/image?filename=fjord&width=200&height=200');
